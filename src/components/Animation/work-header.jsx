...
const WorkHeader = ({ title, service, date, site }) => {
  return (
    <motion.div
      id="header"
      className="redd flex h-[50vh] w-full flex-col items-start justify-end gap-2 overflow-hidden border-b pb-5 laptop:flex-row desktop:items-end desktop:justify-start desktop:gap-10"
    >
      <motion.h1
        initial={{ y: "120%" }}
        whileInView={{ y: "0%", transition: { delay: 0.1, ease: "easeInOut" } }}
        viewport={{ once: true }}
        className="text-5xl font-black desktop:text-8xl"
      >
        {title}
      </motion.h1>
      <motion.h1
        ...
        viewport={{ once: true }}
        className="text-neutral-500 desktop:text-3xl"
      >
        {service}
      </motion.h1>
      <motion.h1
